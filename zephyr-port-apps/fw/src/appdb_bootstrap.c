...
// Cheap idempotency check: an existing file of the right size is treated as
// already installed. Avoids re-reading the whole PBW on every boot; a truncated
// or partial prior write mismatches on size and is reinstalled below.
static bool prv_code_bank_present(const char *filename) {
  int fd = pfs_open(filename, OP_FLAG_READ, FILE_TYPE_STATIC, 0);
  if (fd < S_SUCCESS) {
    return false;
  }
  const bool present = pfs_get_file_size(fd) == (int)sliding_text_emery_pbw_len;
  (void)pfs_close(fd);
  return present;
}
...
static bool prv_install_code_bank(AppInstallId install_id) {
  ...
  if (prv_code_bank_present(filename)) {
