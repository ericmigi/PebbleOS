...
static FwSlot prv_get_ota_target(FwOtaImageType image_type) {
  if (image_type == FwOtaImageRecovery) {
    return (FwSlot){
        .begin = FLASH_REGION_SAFE_FIRMWARE_BEGIN,
        .end = FLASH_REGION_SAFE_FIRMWARE_END,
    };
  }

  const FwBootSelection running = fw_boot_select();
  if (running == FwBootSelectionNormalSlot0) {
    return (FwSlot){
  ...
  return (FwSlot){
      .begin = FLASH_REGION_FIRMWARE_SLOT_0_BEGIN,
      .end = FLASH_REGION_FIRMWARE_SLOT_0_END,
  };
}
